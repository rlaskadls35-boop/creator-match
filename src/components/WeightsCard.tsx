  ...
  return (
    <section className="card weights">
      <div className="weights__head">
        <div className="weights__summary">
          <h2 className="weights__title">매칭 점수 비중 조절</h2>
          <div className="weights__values" aria-label="현재 매칭 점수 비중">
            {METRIC_KEYS.map((k) => (
              <span key={k}>{METRIC_LABEL[k]} <strong>{draft[k] ?? '—'}%</strong></span>
            ))}
          </div>
        </div>
        <div className="weights__head-actions">
          <span className="weights__state">{same ? '저장된 기준' : '저장하지 않은 변경'}</span>
          <button
            type="button"
            className="weights__toggle"
            aria-expanded={open}
            aria-controls="weights-details"
            onClick={() => setOpen((current) => !current)}
          >
            {open ? '접기' : '비중 조절'}
            <ChevronDownIcon className="weights__toggle-icon" aria-hidden="true" />
          </button>
        </div>
      </div>
